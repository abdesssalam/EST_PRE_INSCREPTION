    ...
    public function get_list_principale($fl){
        try{
            ...
            for($i=0; $i<count($row1);$i++){
                $fb = $row1[$i];
                $sql = "SELECT fl.IDFelier, e.IDEtudiant, e.CNE,ut.nom,ut.prenom ,t.initiule as bac ,fl.intitilue as felier from etudiant as e JOIN utilisateur as ut on e.IDEtudiant=ut.idUser JOIN bac as b on b.IDEtudiant=e.IDEtudiant join typebac as t on t.IDType=b.type JOIN insception AS ic on ic.IDEtidiant=e.IDEtudiant JOIN felier as fl on fl.IDFelier=ic.IDFelier WHERE ic.admis=0 AND ic.IDFelier=:F and b.type=:T order by b.noteG  DESC limit {$fb['nb']}";
